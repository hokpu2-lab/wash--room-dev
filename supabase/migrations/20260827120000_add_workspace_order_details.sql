create function public.get_workspace_order_detail(target_laundry_order_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
create function public.get_workspace_snapshot_with_details(
...
declare
  ...
  order_details jsonb;
begin
  snapshot := public.get_workspace_snapshot(
    target_site_id, period_start, period_end,
    order_limit, order_offset, order_query
  );

  if coalesce(snapshot ->> 'outcome', '') <> 'ok' then
    return snapshot;
  end if;

  select coalesce(
    jsonb_agg(public.get_workspace_order_detail((item ->> 'id')::uuid)),
    '[]'::jsonb
  )
  into order_details
  from jsonb_array_elements(
    coalesce(snapshot -> 'queue' -> 'items', '[]'::jsonb)
  ) as item;

  return jsonb_set(snapshot, '{order_details}', order_details, true);
end;
$$;

revoke all on function public.get_workspace_order_detail(uuid),
  public.get_workspace_snapshot_with_details(uuid, timestamptz, timestamptz, integer, integer, text)
