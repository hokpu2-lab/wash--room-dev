...
begin
  ...
  if found then
    ...
    return query select existing.laundry_batch_id, existing.completed_stage_run_id,
      ...
      case when existing.reason_code = 'procedure_completed' then 'awaiting_cart' else 'not_started' end,
  ...
  if next_stage_id is null then
    ...
    set status = 'awaiting_cart', completed_at = null, active_stage_run_id = null, updated_at = now()
    where id = batch.id;
    return query select batch.id, current_run.id, equipment.id, batch.current_stage_order,
      false, 'applied'::text, 'awaiting_cart'::text, 'procedure_completed'::text;
    return;
  end if;
  update public.laundry_batches
...
begin
  for repair in
    ...
    join public.laundry_orders as order_record
      ...
      and order_record.status = 'ready_for_pickup'
    where batch.status = 'completed'
  loop
    update public.laundry_batches
    set status = 'awaiting_cart', completed_at = null, updated_at = now()
    ...
    insert into public.authorization_audit_events(
      ...
      'laundry_batch_reopened_for_source_loading', 'succeeded', '修正烘乾完成後跳過來源車裝車的批次',
      jsonb_build_object('batch_status', repair.batch_status, 'order_status', repair.order_status),
      jsonb_build_object('batch_status', 'awaiting_cart', 'order_status', repair.order_status)
    );
  end loop;

  update public.laundry_orders as order_record
  set status = 'in_process', updated_at = now()
  where order_record.status = 'ready_for_pickup'
    and exists (
      select 1
      from public.laundry_batch_sources as source
      join public.laundry_batches as batch on batch.id = source.shared_batch_id
      where source.laundry_order_id = order_record.id
        and source.load_status = 'pending'
        and batch.status = 'awaiting_cart'
    );
end
$$;
